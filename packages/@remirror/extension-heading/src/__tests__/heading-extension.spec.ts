...
describe('schema', () => {
  ...
  it('defaults to level 1', () => {
    expect(prosemirrorNodeToHtml({ node: h1('Heading'), schema })).toBe('<h1>Heading</h1>');
  });

  it('changes level based on attributes', () => {
    expect(prosemirrorNodeToHtml({ node: h2('Heading'), schema })).toBe('<h2>Heading</h2>');
    expect(prosemirrorNodeToHtml({ node: h3('Heading'), schema })).toBe('<h3>Heading</h3>');
  });
