...
> {
  ...
  /**
   * The store is a shared object that's internal to each extension. It includes often used
  ...
  replaceChildExtension(constructor: AnyExtensionConstructor, extension: this['~E']): void {
    ...
    this.#extensions = this.extensions.map((currentExtension) =>
 ...
 * extension.
 */
export type GetPlainNameUnion<Type> = Type extends AnyPlainExtension ? GetNameUnion<Type> : never;
