...
export function transformExtensions<RawExtensions extends AnyExtension>(
  ...
  // This is the holder for the sorted and cleaned extensions returned by this function.
...
function gatherRawExtensions<ExtensionUnion extends AnyExtension>(
  ...
  // Override the priority if the user has done so in the settings passed to
  // the [[`RemirrorManager`]].
  ...
  // Check if there are any children extensions to be added an if not move
  // onto the next provided extension.
  ...
  for (const child of childExtensions) {
    // Recursively gather all the children extension from the current extension level.
